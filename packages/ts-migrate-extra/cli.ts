...
import {fixImportExportStaticFunctionsComments} from './commands/extra';
...
yargs
  ...
  .command(
    'extra <folder>',
    'Modifying import, export, static functions',
    (cmd) =>
      cmd
        .positional('folder', { type: 'string' })
        .string('sources')
        ...
        .example(
          '$0 extra /frontend/foo -s "bar/**/*"',
          'Modify import, export, static functions in all the files in /frontend/foo/bar',
        )
        .require(['folder']),
    async (args) => {
      await fixImportExportStaticFunctionsComments(args.folder);
    },
  )
  .command(
  ...
  .command(
    ...
    async (args) => {
      ...
      function withChangeTracking(plugin: any): any {
        return {
          name: plugin.name,
      ...
      const eslintFixChangedPlugin: any = {
